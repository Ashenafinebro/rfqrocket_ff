...
def process_chunk(chunk):
    ...
    system_prompt = (
        "You are an AI assistant working on behalf of a procurement intermediary (middleman) representing a government client. "
        "Your task is to convert government solicitations into clean, vendor-safe RFQs without revealing the identity of the client.\n\n"
        "Instructions:\n"
        "- Extract only essential quote-related details.\n"
        "- Eliminate redundant ideas, repeated phrases, and duplicate concepts. If a requirement or evaluation criterion is listed multiple ways, consolidate and summarize it clearly once.\n"
        "- Do not include any references to government agencies, officers, locations, contract numbers, or compliance clauses (e.g., FAR, DFARS, SAM).\n"
        "- Write clearly and concisely. Use neutral commercial language. The output must be professional, minimal, and not verbose.\n"
        "- Return the RFQ content using the following exact JSON format. Ensure all sections are populated, but avoid duplication and unnecessary elaboration.\n\n"
        "Return only the JSON — no explanations, no extra formatting."
    )

    user_prompt = f"""
    Use the government solicitation text provided below to generate a vendor-facing RFQ based on the instructions.
    ...
    Strictly return only a clean JSON object with this structure:
    ...
    "KEY_DATES_AND_DOCUMENTS": {{}}
    }}

    Avoid repeating the same ideas in different forms. Summarize all content efficiently and cleanly.
    """
        ...
        for item in content:
            if isinstance(item, dict):
                for key, value in item.items():
                    p = doc.add_paragraph()
                    p.add_run(f"{key}: ".replace("_", " ")).bold = True
                    p.add_run(str(value))
            else:
